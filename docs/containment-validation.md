...
flowchart TD
    ...
    C --> D["EC2 quarantine group"]
    C --> E["DynamoDB incident record"]
    C --> F["SNS notification and CloudWatch logs"]
```
...
4. The target instance security group is replaced with a quarantine security group that has no ingress or egress rules.
5. The instance receives the `IncidentStatus=contained` tag while remaining powered on for investigation.
6. The incident record is persisted in DynamoDB, an SNS notification is published, and structured events are written to CloudWatch Logs.
7. Reprocessing the same incident returns `already_contained` without repeating the mutation or changing the original completion time.
...
| Auditability | Before/after state, timestamps, result, and failure details are persisted |
